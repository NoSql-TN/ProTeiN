...
list_of_queries = ["MATCH ()-[r:similarity]->() RETURN COUNT(r) AS nombreTotalRelations;", 
                    ...
                    "MATCH ()-[r:similarity]->() WITH r.jaccardID AS jaccardID WITH CASE WHEN jaccardID >= 0 AND jaccardID < 0.1 THEN '0-0.1' WHEN jaccardID >= 0.1 AND jaccardID < 0.2 THEN '0.1-0.2' WHEN jaccardID >= 0.2 AND jaccardID < 0.3 THEN '0.2-0.3' WHEN jaccardID >= 0.3 AND jaccardID < 0.4 THEN '0.3-0.4' WHEN jaccardID >= 0.4 AND jaccardID < 0.5 THEN '0.4-0.5' WHEN jaccardID >= 0.5 AND jaccardID < 0.6 THEN '0.5-0.6' WHEN jaccardID >= 0.6 AND jaccardID < 0.7 THEN '0.6-0.7' WHEN jaccardID >= 0.7 AND jaccardID < 0.8 THEN '0.7-0.8' WHEN jaccardID >= 0.8 AND jaccardID < 0.9 THEN '0.8-0.9' WHEN jaccardID >= 0.9 AND jaccardID <= 1 THEN '0.9-1.0' ELSE 'Out of range' END AS jaccardRange RETURN jaccardRange, COUNT(*) AS count ORDER BY jaccardRange;",
                    "MATCH (p:Protein)-[r:similarity]->(q:Protein) WITH p, AVG(r.jaccardID) AS avgJaccard RETURN CASE WHEN avgJaccard >= 0 AND avgJaccard < 0.1 THEN '0.0-0.1' WHEN avgJaccard >= 0.1 AND avgJaccard < 0.2 THEN '0.1-0.2' WHEN avgJaccard >= 0.2 AND avgJaccard < 0.3 THEN '0.2-0.3' WHEN avgJaccard >= 0.3 AND avgJaccard < 0.4 THEN '0.3-0.4' WHEN avgJaccard >= 0.4 AND avgJaccard < 0.5 THEN '0.4-0.5' WHEN avgJaccard >= 0.5 AND avgJaccard < 0.6 THEN '0.5-0.6' WHEN avgJaccard >= 0.6 AND avgJaccard < 0.7 THEN '0.6-0.7' WHEN avgJaccard >= 0.7 AND avgJaccard < 0.8 THEN '0.7-0.8' WHEN avgJaccard >= 0.8 AND avgJaccard < 0.9 THEN '0.8-0.9' WHEN avgJaccard >= 0.9 AND avgJaccard <= 1.0 THEN '0.9-1.0' ELSE 'Other' END AS jaccardRange, COUNT(p) AS nodeCount ORDER BY jaccardRange;",
                    "MATCH (p:Protein) WHERE p.interPro RETURN size(p.interPro) AS numberOfInterPro, COUNT(p) AS numberOfProteins ORDER BY numberOfInterPro; ",
                    "MATCH (p:Protein) WHERE p.interPro IS NOT NULL AND SIZE(p.interPro) > 0 UNWIND p.interPro AS interProElement  WITH interProElement, COUNT(*) AS frequency RETURN interProElement, frequency ORDER BY frequency DESC LIMIT 11;",
                    ...
                    "MATCH (p:Protein) WHERE p.organism = 'Homo sapiens (Human)' AND p.interPro IS NOT NULL AND SIZE(p.interPro) > 0 UNWIND p.interPro AS interProElement WITH p.organism AS organism, interProElement, COUNT(*) AS frequency RETURN organism, interProElement, frequency ORDER BY organism, frequency DESC LIMIT 2;",
                    ...
                    "MATCH (p:Protein) WHERE p.organism = 'Mus musculus (Mouse)' AND p.interPro IS NOT NULL AND SIZE(p.interPro) > 0 UNWIND p.interPro AS interProElement WITH p.organism AS organism, interProElement, COUNT(*) AS frequency RETURN organism, interProElement, frequency ORDER BY organism, frequency DESC LIMIT 2;",
                    ...
                    "MATCH (p:Protein) WHERE p.organism = 'Arabidopsis thaliana (Mouse-ear cress)' AND p.interPro IS NOT NULL AND SIZE(p.interPro) > 0 UNWIND p.interPro AS interProElement WITH p.organism AS organism, interProElement, COUNT(*) AS frequency RETURN organism, interProElement, frequency ORDER BY organism, frequency DESC LIMIT 2;",
                    ...
                    "MATCH (p:Protein) WHERE p.organism = 'Rattus norvegicus (Rat)' AND p.interPro IS NOT NULL AND SIZE(p.interPro) > 0 UNWIND p.interPro AS interProElement WITH p.organism AS organism, interProElement, COUNT(*) AS frequency RETURN organism, interProElement, frequency ORDER BY organism, frequency DESC LIMIT 2;",
